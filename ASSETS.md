...
| WORD LADDER deny list | data/word-lists/deny.txt | Self authored, seeded from a public profanity list | Repo license | Build time input only, never served. Four letter vulgar words and slurs subtracted from both the accepted and familiar lists at derivation so no such word is ever a valid rung or an endpoint. Reviewed by a person |
