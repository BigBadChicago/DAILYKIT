...
export function rayTarget(
  ...
  const step = STEP[dir] as readonly [number, number];
...
export function buildGeometry(clues: ClueLayout): Geometry {
  ...
  for (const cell of blankCells) {
    for (const dir of DIRECTIONS) {
      ...
      candidates[cell] = (candidates[cell] as number) | (1 << dir);
...
export function candidateList(
  ...
  const mask = geometry.candidates[cell] ?? 0;
  const out: Direction[] = [];
...
export function isCandidate(
  ...
  return ((geometry.candidates[cell] ?? 0) & (1 << dir)) !== 0;
}
...
export function nextCandidate(
  ...
  if (current === null) return list[0] as Direction;
  const at = list.indexOf(current);
  if (at < 0) return list[0] as Direction;
  return at + 1 < list.length ? (list[at + 1] as Direction) : null;
}
...
export function arrivals(
  ...
  for (const cell of geometry.blankCells) {
    ...
    if (target !== NO_TARGET) counts[target] = (counts[target] as number) + 1;
  }
  return counts;
}
...
export type Propagation =
  | {
      ...
      /** The 1 based round in which each cell was assigned, 0 on clue cells.
       *  The intensity measure sums these over the blanks. VECTOR.md 9.2. */
...
export function propagate(geometry: Geometry): Propagation {
  ...
  const rounds: number[] = new Array<number>(CELLS).fill(0);
  ...
  const live = (supplier: Supplier): boolean =>
    ...
    ((domain[supplier.cell] ?? 0) & (1 << supplier.dir)) !== 0;
  ...
  while (remaining > 0) {
    ...
    for (const key of remove) {
      ...
      if (assigned[cell] !== null) continue;
      if (((domain[cell] ?? 0) & (1 << dir)) !== 0) {
        domain[cell] = (domain[cell] as number) & ~(1 << dir);
        productive = true;
      }
    }
  ...
  return { kind: "resolved", arrows: assigned, depth, opening, rounds };
}
