...
DAILYKIT is a suite of twelve daily puzzle games on one static site, in the genre
of Wordle and Connections. One puzzle a day, identical for every player
worldwide, a spoiler free share block, and a streak. Three games are live today,
POKER GRID, VECTOR and CIPHER. ROTATE LOCK and DIFFERENCE RELAY are built and
planned. The next four, in build order, are the word games added 2026-09-19:
LETTER TRAIL, WORD LADDER, PANGRAM and FIVE LETTERS. TURN TABLE, RING BALANCE and
ORDER OF OPERATIONS follow them. All are carried in `src/shell/registry.ts`. The
engine and the games are separated by a contract so that a new game is one
directory plus one entry file and zero engine changes.

The suite was five games until 2026-09-13, eight from then, and twelve from
2026-09-19. Both are stated deviations from the charter's Section 0 and
requirement 7.3.1, recorded in ARCHITECTURE2.md section 56 under Slate
