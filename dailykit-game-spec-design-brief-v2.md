...
```
NUMBER OF CONCEPTS WANTED:       between fifteen and twenty
...
5. **[FEATURE REQUIREMENT: SOCIAL TELEMETRY AND EMERGENT ARTIFACTS]** The game must include an end of session summary system that converts player actions into a shareable, visual artifact. Every concept must select at least two of the nine patterns below and implement them in the game design.

**Core telemetry mechanics**

1. **Asymmetric spatial telemetry sharing.** Map player decision paths on a shared board or puzzle space without revealing underlying solution values.
2. **Deterministic output generation.** Define an absolute 1:1 rule mapping player inputs or states directly to distinct visual tokens, for example colour blocks, symbols, or vector shapes.
3. **Spoiler free replay visual.** Redact explicit text or target solutions while visually preserving attempt counts, mistakes, spikes in difficulty, and turning points.
4. **Emergent player profile, the fingerprint.** Ensure the final visual artifact creates a distinct pattern that acts as a unique signature of that player's specific run.

**Additional social and data mechanics**

5. **Asymmetric consensus heatmaps.** Display a post session breakdown comparing the player's specific choices against global community averages without exposing upcoming gameplay pathing.
6. **Seed based synchronous run visualization.** Track and map divergent player paths across a shared, deterministic random seed.
7. **Micro replay path tracing.** Render a simplified geometric or vector trace of player movement and action density over the board state.
8. **Playstyle archetype classification.** Translate run data ratios such as speed, accuracy, and risk factor into a shareable title, badge, or behavioral persona card.
9. **Comparative friction graphs.** Generate a timeline or step chart highlighting time per turn or struggle points throughout the session.

**Output requirements for every concept's design spec.** Identify which specific mechanics above are used. Define the underlying data structure logged during play, for example an array of turn states or action vectors. Define the mapping engine, meaning how raw input becomes a visual token or archetype. Provide the export payload format: a text and emoji string for the clipboard, plus a graphic card visual spec for image sharing.

The fifth property is the one that matters most. This artifact is the platform's only distribution: there is no advertising, no feed, and no recommendation engine. A game whose result cannot be encoded spoiler free is not a daily game, however good it is otherwise. Section 10 defines the vocabulary, grammars, leak test, and payload that this requirement is implemented against.
...
The share block is the emergent artifact required by property five in Section 2, and it is the platform's only distribution, so it is the most constrained thing you will design and also the place where a good concept most often earns its life. The constraints below are narrow where they have to be and open where they do not. Read the distinction, because concepts routinely refuse themselves over a rule that is not actually there.

Property five names nine telemetry patterns and requires each concept to implement at least two. This section is where those patterns become real: the vocabulary they draw glyphs from (10.2), the grammars they render into (10.4), the leak test they must survive (10.6), and the pipeline they are specified as (10.8a). A pattern that cannot pass the leak test is rejected however clever it is. Rough pairing of pattern to grammar, not a rule, only a starting point: asymmetric spatial telemetry and micro replay tracing suit grammar E, board snapshot; deterministic output generation and spoiler free replay suit grammars A and B; the fingerprint is a property of whichever grammar you pick rather than a grammar of its own; consensus heatmaps and synchronous run visualization usually need grammar D or E; archetype classification lives on the title line under 10.7; friction graphs suit grammar C or a step read of B.
...
### 10.8a The telemetry pipeline the concept must specify

Property five requires three stages beyond the drafted block. State each concretely; a field that says "an array of moves" without a shape has not been designed.

- **Data structure logged during play.** The record the session writes to on every action, named as a type with its shape, not a paragraph. The right level of detail: `TurnState[]` where each `TurnState` is `{ index, tokenQuality, msElapsed }`; or `ActionVector[]` where each vector is `{ cell, direction, accepted }`; or a fixed `Profile` of `{ partId -> outcome }`. This is what the mapping engine consumes and what a future replay would read, so it captures everything the artifact needs and nothing that would leak if read out of storage.
- **Mapping engine.** The deterministic function from that log to tokens and, where used, to an archetype. State it as raw input to visual token: which logged field selects the glyph, from which token family, under which grammar, and the exact thresholds or 1:1 rule. If the archetype pattern is used, give the ratio to archetype table. It must be pure and reproducible from the log alone, which is what makes deterministic output generation actually deterministic.
- **Export payload.** Two forms. First, the text and emoji string for the clipboard, which is the drafted block itself under the nine line and rendering rules. Second, a one line graphic card visual spec for image sharing: the fingerprint rendered larger, its canvas aspect ratio, what the tokens become as vector shapes or colour blocks, and where the title and URL sit. The card is optional to ship but its spec is required, because a fingerprint that only works at emoji resolution and falls apart as an image is a weaker artifact than one that reads at both sizes.

One platform caveat, because two of the nine patterns tempt it. Section 4 C3 forbids a server, accounts, and live data. A consensus heatmap or any comparison against community averages is therefore permitted only if the aggregate is a precomputed distribution baked into the day's manifest in continuous integration, never a live query. A synchronous run visualization compares two players who already hold the same seed offline; it never phones home. Any pattern that needs a live network round trip is rejected under C3.
...
SHARE BLOCK (SOCIAL TELEMETRY ARTIFACT):
...
shape. Then one line each on grammar, leak test, and range, per 10.8. Then the
telemetry declarations required by property five in Section 2, per 10.8a:
  TELEMETRY MECHANICS: which two or more of the nine patterns this uses.
  DATA STRUCTURE: the logged type and its shape.
  MAPPING ENGINE: the deterministic rule from log field to token, plus the ratio
    to archetype table if the archetype pattern is used.
  EXPORT PAYLOAD: the clipboard string (the block above) and a one line graphic
    card visual spec for image sharing.
  FINGERPRINT: one sentence on what makes two same tier runs look different.]
