...
function hasSolutionWith(layout: ClueLayout, fixedCell: number, fixedDir: Direction): boolean {
  ...
  const place = (cell: number, dir: Direction): boolean => {
    const target = local.targets[cell * 4 + dir] as number;
    if ((counts[target] as number) + 1 > (layout[target] as number)) return false;
    arrows[cell] = dir;
  ...
  const lift = (cell: number): void => {
    ...
    const t = local.targets[cell * 4 + dir] as number;
