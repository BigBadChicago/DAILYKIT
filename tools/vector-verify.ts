...
function hasSolutionWith(
  ...
  const value = (clue: number): number => layout[clue] as number;

  const place = (cell: number, dir: Direction): boolean => {
    const target = geometry.targets[cell * 4 + dir] as number;
    if ((counts[target] as number) + 1 > value(target)) return false;
    assigned[cell] = dir;
    counts[target] = (counts[target] as number) + 1;
    return true;
  };
  const lift = (cell: number): void => {
    const dir = assigned[cell] as Direction;
    const target = geometry.targets[cell * 4 + dir] as number;
    counts[target] = (counts[target] as number) - 1;
    assigned[cell] = null;
  };

  /* The propagation pruning VECTOR.md section 10 relies on to keep the search
     near its root. Every clue must still be able to reach its value from the
     arrows already placed plus the unassigned cells that can still supply it.
     A per clue bound only, so it never prunes a branch that could complete. */
  const feasible = (): boolean => {
    for (const clue of geometry.clueCells) {
      const need = value(clue) - (counts[clue] as number);
      if (need < 0) return false;
      if (need === 0) continue;
      let supply = 0;
      for (const supplier of geometry.suppliers.get(clue) ?? []) {
        if (assigned[supplier.cell] === null) supply += 1;
      }
  ...
  const walk = (): boolean => {
    ...
    /* Most constrained unassigned cell first. A cell whose every candidate is
       already full is a dead end caught here rather than one level deeper. */
    let pick = -1;
    ...
    for (const cell of geometry.blankCells) {
      ...
      for (const dir of candidateList(geometry, cell)) {
        const target = geometry.targets[cell * 4 + dir] as number;
        if ((counts[target] as number) < value(target)) live += 1;
      }
    ...
    if (pick === -1) return satisfies(geometry, assigned);

    for (const dir of candidateList(geometry, pick)) {
      if (!place(pick, dir)) continue;
      if (walk()) return true;
      lift(pick);
    }
    return false;
...
function main(): void {
  ...
  for (const [puzzleNumber] of entries) {
    const b = bandForPuzzle(puzzleNumber);
    spread[b] = (spread[b] as number) + 1;
  }
